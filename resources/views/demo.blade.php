...
<head>
  ...
  <link rel="stylesheet" href="{{ url('/themes/' . $GLOBALS['themeName'] . '/share.button.css') }}">
  @if(theme('use_default_buttons') == "true")
  <link rel="stylesheet" href="{{ asset('assets/wayvio/css/brands.css') }}">
  @else
  <link rel="stylesheet" href="{{ url('/themes/' . $GLOBALS['themeName'] . '/brands.css') }}">
  @endif
  <link rel="stylesheet" href="{{ url('/themes/' . $GLOBALS['themeName'] . '/skeleton-auto.css') }}">
@if(file_exists(base_path('themes/' . $GLOBALS['themeName'] . '/animations.css')))
  <link rel="stylesheet" href="{{ url('/themes/' . $GLOBALS['themeName'] . '/animations.css') }}">
@else
      ...
      <div class="column" style="margin-top: 15%">
        ...
        @if(config('advanced-config.use_custom_buttons') == 'true')
                ...
                @foreach($array as $button)
                 ...
                 @php $hideIcon = ($button['icon'] ?? '') === 'ls-hidden-icon'; @endphp
                 <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a class="button button-{{ $button['button'] }} button button-hover icon-hover" rel="noopener noreferrer nofollow" href="{{ $button['link'] }}" @if(theme('open_links_in_same_tab') != "true")target="_blank"@endif >@if(!$hideIcon)@if($button['icon'] == 'llc')<img alt="button-icon" class="icon hvr-icon" src="{{ asset('\/assets/wayvio/icons\/')}}llc.svg">@else<i style="color: {{$button['icon']}}" class="icon hvr-icon fa {{$button['icon']}}"></i>@endif @endif{{ $button['title'] }}</a></div>
                 @elseif($button['button'] === "custom" and $button['custom_css'] != "")
                 @php $hideIcon = ($button['icon'] ?? '') === 'ls-hidden-icon'; @endphp
                 <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a class="button button-hover icon-hover" style="{{ $button['custom_css'] }}" rel="noopener noreferrer nofollow" href="{{ $button['link'] }}" @if(theme('open_links_in_same_tab') != "true")target="_blank"@endif >@if(!$hideIcon)@if($button['icon'] == 'llc')<img alt="button-icon" class="icon hvr-icon" src="{{ asset('\/assets/wayvio/icons\/')}}llc.svg">@else<i style="color: {{$button['icon']}}" class="icon hvr-icon fa {{$button['icon']}}"></i>@endif @endif{{ $button['title'] }}</a></div>
                 @elseif($button['button'] === "buy me a coffee")
                 <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a class="button button-coffee button button-hover icon-hover" rel="noopener noreferrer nofollow" href="{{ $button['link'] }}" @if(theme('open_links_in_same_tab') != "true")target="_blank"@endif ><img alt="button-icon" class="icon hvr-icon" src="@if(theme('use_custom_icons') == "true"){{ url('themes/' . $GLOBALS['themeName'] . '/extra/custom-icons')}}/coffee{{theme('custom_icon_extension')}} @else{{ asset('\/assets/wayvio/icons\/')}}coffee.svg @endif">Buy me a Coffee</a></div>
                 ...
                 @php $hideIcon = ($button['icon'] ?? '') === 'ls-hidden-icon'; @endphp
                 <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a class="button button-custom_website button button-hover icon-hover" rel="noopener noreferrer nofollow" href="{{ $button['link'] }}" @if(theme('open_links_in_same_tab') != "true")target="_blank"@endif >@if(!$hideIcon)<img alt="button-icon" class="icon hvr-icon" src="@if(file_exists(base_path("assets/favicon/icons/").localIcon($button['id']))){{url('assets/favicon/icons/'.localIcon($button['id']))}}@else{{getFavIcon($button['id'])}}@endif">@endif{{ $button['title'] }}</a></div>
                 @elseif($button['button'] === "custom_website" and $button['custom_css'] != "")
                 @php $hideIcon = ($button['icon'] ?? '') === 'ls-hidden-icon'; @endphp
                 <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a class="button button-hover icon-hover" style="{{ $button['custom_css'] }}" rel="noopener noreferrer nofollow" href="{{ $button['link'] }}" @if(theme('open_links_in_same_tab') != "true")target="_blank"@endif >@if(!$hideIcon)<img alt="button-icon" class="icon hvr-icon" src="@if(file_exists(base_path("assets/favicon/icons/").localIcon($button['id']))){{url('assets/favicon/icons/'.localIcon($button['id']))}}@else{{getFavIcon($button['id'])}}@endif">@endif{{ $button['title'] }}</a></div>
                 ...
                 <h2>{{ $button['title'] }}</h2>
                 @else
                 <div style="--delay: {{ $initial++ }}s" class="button-entrance"><a class="button button-{{ $button['button'] }} button button-hover icon-hover" @if($button['link'] != '') href="{{ $button['link'] }}" target="_blank"@endif><img alt="button-icon" class="icon hvr-icon" src="{{ asset('\/assets/wayvio/icons\/') . $linkName }}.svg">{{ ucfirst($linkName) }}</a></div>
                 @endif
                @endforeach
        @else
                <div style="--delay: {{ $initial++ }}s" class="button-entrance"><div class="button button-github button button-hover icon-hover"><img alt="button-icon" class="icon hvr-icon" src="{{ asset('assets/wayvio/icons/github.svg') }}">Github</div></div>
                <div style="--delay: {{ $initial++ }}s" class="button-entrance"><div class="button button-twitter button button-hover icon-hover"><img alt="button-icon" class="icon hvr-icon" src="{{ asset('assets/wayvio/icons/twitter.svg') }}">Twitter</div></div>
                <div style="--delay: {{ $initial++ }}s" class="button-entrance"><div class="button button-instagram button button-hover icon-hover"><img alt="button-icon" class="icon hvr-icon" src="{{ asset('assets/wayvio/icons/instagram.svg') }}">Instagram</div></div>
        @endif
