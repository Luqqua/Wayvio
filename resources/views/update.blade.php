...
@push('updater-body')
    ...
    @php
        // Must end with '/'
        $betaServer          = env('BETA_SERVER', 'https://beta.wayvio.example/');
        $betaPreUpdateServer = env('BETA_PRE_UPDATE_SERVER', 'https://pre-update.wayvio.example/beta/');
        $updateServer        = env('UPDATE_SERVER', 'https://update.wayvio.example/');
        $versionServer       = env('VERSION_SERVER', 'https://version.wayvio.example/');
        $preUpdateServer     = env('PRE_UPDATE_SERVER', 'https://pre-update.wayvio.example/');
        $repositoryUrl       = env('REPOSITORY_URL', 'https://github.com/wayvio/wayvio/');
        ...
        @if ((auth()->user()->role == 'admin' && $Vgit > $Vlocal) || $isBeta)
            @if (empty($_SERVER['QUERY_STRING']))
                <div class="logo-container fadein">
                    <img class="logo-img" src="{{ asset('assets/wayvio/images/logo.svg') }}" alt="Logo">
                </div>
                ...
                <div class="row">
                    ...
                    <a class="btn" href="https://wayvio.example/update" target="_blank">
