...
<h5 style="margin-top:50px">{{__('messages.ALLOW_USER_HTML.title')}}</h5>
<p class="text-muted">Disabled for security reasons. User-provided HTML is not allowed.</p>
...
@endphp
    ...
    <div class="input-group">
    <input type="text" style="border-radius:.25rem;max-width:600px" class="form-control" name="value" value="{{$configValue}}" autocomplete="new-password" autocapitalize="off" autocorrect="off" spellcheck="false" inputmode="url" aria-autocomplete="none" data-lpignore="true" data-1p-ignore="true" readonly onfocus="this.removeAttribute('readonly')" onpointerdown="this.removeAttribute('readonly')" ontouchstart="this.removeAttribute('readonly')" onblur="this.setAttribute('readonly', 'readonly')">
    <input type="hidden" name="_token" value="{{csrf_token()}}">
	<button  type="submit" class="btn btn-primary">{{__('messages.Apply')}}</button>
    </div></div>
    </form>
