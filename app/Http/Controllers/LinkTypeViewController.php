...
class LinkTypeViewController extends Controller
{
    public function getParamForm($typename, $linkId = 0)
    {
        ...
        if ($linkId) {
            $link = Link::query()->where('id', (int) $linkId)->firstOrFail();
            if (Auth::check()) {
                Gate::forUser(Auth::user())->authorize('view', $link);
            }

            $data['title'] = $link->title;
            $data['link'] = $link->link;
            $data['custom_icon'] = $link->custom_icon ?? '';
            $data['is_adult'] = (bool) ($link->is_adult ?? false);
            if (Route::currentRouteName() != 'showButtons') {
                $data['button_id'] = $link->button_id;
            }

            // Keep legacy links with the old default icon behaving like "website icon".
            if ((int) $data['button_id'] === 2 && $data['custom_icon'] === 'fa-external-link') {
                $data['custom_icon'] = '';
            }
        ...
        if ($typename === 'predefined') {
            ...
            return response(view('components.pageitems.predefined-form', $data))
        ...
        return response(view($typename . '.form', $data))
            ->header('Cache-Control', 'no-store, private');
    }
